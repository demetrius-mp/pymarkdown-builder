...
PACKAGE_NAME = "pymarkdown_builder"
API_PAGE_DIR = "api"
...
for path in sorted(Path("src").rglob("*.py")):
    ...
    if not str(module_path).startswith(PACKAGE_NAME):
        continue

    doc_path = path.relative_to("src").with_suffix(".md")
    full_doc_path = Path(API_PAGE_DIR, doc_path)
...
with mkdocs_gen_files.open(f"{API_PAGE_DIR}/SUMMARY.md", "w") as nav_file:
